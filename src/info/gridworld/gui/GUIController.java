...
public class GUIController<T>
{
    ...
    private void locationClicked()
    {
        ...
        {
            Piece occupant = (Piece)world.getGrid().get(loc);
            if((occupant == null || game.currentPlayer() != occupant.getPlayer()) && game.selectedPiece() != null)
            {
            	if(game.selectedPiece().canMoveTo(loc))
            	{
            		if(!game.makeMove(loc))
            		{
            			world.setMessage("Invalid move: you will be in check.");
            		}
            		else
            		{
            			String message = "";
            			Piece[] threats = game.currentPlayer().inCheck();
            			if(threats.length > 1)
            			{
            				if(game.canMoveKing())
            				{
            			...
            			{
            				if(game.canMoveKing() || game.canMoveOthers(threats[0]))
            				{
            					message = "Check";
            				}
            				else
            	...
            	{
            		world.setMessage("Invalid move");
            	}
            }
            else if(occupant == null || occupant.getPlayer() == game.currentPlayer())
            {
            	game.selectPiece(occupant);
            }
        }
        parentFrame.repaint();
    }
